...
from Evaluation.RankingMetrics import evaluate
...
class CDAE():
    ...
    def __init__(self, sess, num_user, num_item, learning_rate = 0.01, reg_rate = 0.03, epoch = 500, batch_size = 100, verbose = False, T = 3, display_step= 1000):
    ...
    def build_network(self, hidden_neuron = 200, corruption_level = 0.3):
        ...
        self.loss = - tf.reduce_sum(self.rating_matrix * tf.log(self.layer_2) + (1 - self.rating_matrix) * tf.log( 1- self.layer_2))\
                                      + self.reg_rate * (tf.norm(W) + tf.norm(W_prime)  + tf.norm(V) + tf.norm(b) + tf.norm(b_prime))
